...
impl Redis {
    ...
    pub fn push_verification_request(&mut self, verification_request: &crate::account::VerificationRequest) -> Result<(), ()> {
        ...
        redis::cmd("LPUSH").arg("queue:verification_request").arg(payload).query(&mut self.connection).map_err(|_| ())?;
        Ok(())
    }
    ...
    pub fn get_pub_key(&mut self, account_id: String) -> Result<String, ()> {
        redis::cmd("GET").arg(account_id).query(&mut self.connection).map_err(|_| ())
    }
